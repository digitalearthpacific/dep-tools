...
class DepWriter(Writer):
    ...
    def write(self, xr, item_id):
        ...
        self._cog_writer.write(output, id)
        if self._stac_writer is not None:
            self._stac_writer.write(output, id)
...
class DsWriter(DepWriter):
    def __init__(
        ...
        output_value_multiplier: int = 1000,
        ...
        cog_writer = DsCogWriter(
            itempath, write_multithreaded, load_before_write, write_function
        )
